...
public class FactoryReservation {
	 ...
	 */
	public Reservation creerReservation (Date dPR, Date dR, ArrayList<PlageHoraire> plages, int p, Client c, Salle s, int duree) throws ExceptionReservationExistante, SQLException{
		...
		cacheReservation.put(idReservation, s);

		sql = "INSERT INTO RESERVATION (id_reservation, datePriseReservation, dateReservation, plagesHoraires, prix, client, salle, duree)"
				+ " VALUES ('"
				+ idReservation
				+ "', '"
				+ dPR
	 ...
	 * @param idReservation
	 */
	public void supprReservation(Reservation idReservation){
	...
	public ArrayList<Reservation> listeReservationBDD() throws SQLException {
		...
		String sql = "SELECT datePriseReservation, dateReservation, plagesHoraires, prix, client, salle, duree FROM SALLE";
		ResultSet rs = FactorySQL.getInstance().getResultSet(sql);

		while (rs.next()) {
			Date dPR = rs.getDate("datePriseReservation");
			Date dR = rs.getDate("dateReservation");
			ArrayList<PlageHoraire> plages = null;
	 ...
	 * @throws SQLException
	 */
	public Reservation rechercheReservation(int idReservation) throws SQLException		
		...
		String sql = "SELECT datePriseReservation, dateReservation, plagesHoraires, prix, client, salle, duree FROM RESERVATION WHERE id_reservation="+ idReservation;
		ResultSet rs = FactorySQL.getInstance().getResultSet(sql);
		rs.last();
		...
		return new Reservation (dPR, dR,plages, p, c, s, duree);
		}
	}
	 ...
	 * @throws SQLException
	 */
	public Reservation rechercheReservation(Date dR, Client c) throws SQLException{
		//On recherche dans le cache
		int hash = 1;
		hash = hash * 21 + c.hashCode();
		hash = hash * 11 + dR.hashCode();		
		hash = Math.abs(hash);
		Reservation r = cacheReservation.get(hash);
		if (r!=null){
			return r;		
		} else {
		//Sinon on recherche dans la BDD
		String sql = "SELECT datePriseReservation, dateReservation, plagesHoraires, prix, client, salle, duree FROM RESERVATION WHERE client="
		+ c.hashCode() + "AND dateReservation =" + dR;
		ResultSet rs = FactorySQL.getInstance().getResultSet(sql);
		rs.last();
		int nbLigne = rs.getRow();
		if (nbLigne<=0) throw new ExceptionReservationInexistante("La Reservation n'existe pas");
	 ...
	 * @throws SQLException
	 */
	public ArrayList<Reservation> listeReservationDate(Date dR) throws SQLException{
		ArrayList<Reservation> lesReservations = new ArrayList<Reservation>();
		String sql = "SELECT datePriseReservation, dateReservation, plagesHoraires, prix, client, salle, duree FROM SALLE WHERE dateReservation =" +dR;
		ResultSet rs = FactorySQL.getInstance().getResultSet(sql);
	 ...
	 * @throws SQLException
	 */
	public ArrayList<Reservation> listeReservationClient(Client c) throws SQLException{
		ArrayList<Reservation> lesReservations = new ArrayList<Reservation>();
		String sql = "SELECT datePriseReservation, dateReservation, plagesHoraires, prix, client, salle, duree FROM SALLE WHERE client =" +c.hashCode();
		ResultSet rs = FactorySQL.getInstance().getResultSet(sql);
	 ...
	 * @throws SQLException
	 */
	public ArrayList<Reservation> listeReservationSalle(Salle s) throws SQLException{
		ArrayList<Reservation> lesReservations = new ArrayList<Reservation>();
		String sql = "SELECT datePriseReservation, dateReservation, plagesHoraires, prix, client, salle, duree FROM SALLE WHERE salle =" + s.getNom();
		ResultSet rs = FactorySQL.getInstance().getResultSet(sql);
